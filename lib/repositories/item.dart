...
class ItemRepository {
  ...
  static Future<List<String>> _getPartPath(int id) async {
    return await db.transaction(() async {
      final part = await db.managers.parts
      ...
      if (part.parentId != null) {
        ...
        return basePaths.map((path) => StringBuilder.titleBuilder(path, part.name)).toList();
      } else {
        final partVehicles = await db.managers.partVehicles
          .withReferences(
        ...
        return partVehicles.map((name) => StringBuilder.titleBuilder(name, part.name)).toList(); 
      }
    });
  }

  static Future<List<ItemPath>> getItemLinkedPart(int id) async {
    final parts = await db.managers.partItems
      .filter((f) => f.itemId.id.equals(id))
      .get();
